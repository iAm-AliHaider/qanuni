  ...
  return (
    <AppShell><div className="min-h-[100dvh] bg-transparent" dir={dir}>
      ...
      <main className="p-3 md:p-6 max-w-6xl mx-auto space-y-4 page-transition">
         {/* Mobile header */}
         <div className="flex items-center justify-between md:hidden"><h1 className="text-lg font-bold text-slate-900">{t("ctr.mobile_header")}</h1><button onClick={() => setShowForm(true)} className="px-3 py-1.5 rounded-xl bg-emerald-500 text-white text-xs font-semibold">+ {t("ctr.new_contract")}</button></div>

         <div className="grid grid-cols-2 md:grid-cols-4 gap-2 stagger">
           <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-3 border-l-4 border-l-blue-400 animate-slide-up card-hover"><p className="text-xl font-bold text-blue-600">{stats.total || 0}</p><p className="text-[10px] text-slate-400">{t("ctr.total_label")}</p></div>
           <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-3 border-l-4 border-l-emerald-400 animate-slide-up card-hover"><p className="text-xl font-bold text-emerald-600">{stats.active || 0}</p><p className="text-[10px] text-slate-400">{t("ctr.active_label")}</p></div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                   <div className="flex items-center gap-2 mb-1 flex-wrap">
                     ...
                     <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase ${TYPE_COLORS[c.contract_type] || "bg-gray-100 text-gray-500"}`}>{t(`ctr.type_${c.contract_type}`)}</span>
                     <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase ${STATUS_COLORS[c.status] || "bg-gray-100 text-gray-500"}`}>{t(`common.${c.status}`)}</span>
                   </div>
                  <p className="text-sm font-semibold text-slate-900">{c.title}</p>
                   <div className="flex flex-wrap gap-3 mt-1.5 text-[10px] text-slate-400">
                     {c.client_name && <span>{t("common.client")}: {c.client_name}</span>}
                     {c.case_ref && <span>{t("common.case")}: {c.case_ref}</span>}
                     {c.start_date && <span>{c.start_date} → {c.end_date || t("common.ongoing")}</span>}
                   </div>
                   {/* Signature status */}
                   <div className="flex items-center gap-3 mt-2">
                     <span className={`px-2 py-0.5 rounded-lg text-[9px] font-semibold ${c.signed_by_firm ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-slate-50 text-slate-400 border border-slate-200"}`}>{c.signed_by_firm ? t("ctr.firm_signed") : t("ctr.firm_pending")}</span>
                     <span className={`px-2 py-0.5 rounded-lg text-[9px] font-semibold ${c.signed_by_client ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-slate-50 text-slate-400 border border-slate-200"}`}>{c.signed_by_client ? t("ctr.client_signed") : t("ctr.client_pending")}</span>
                   </div>
                </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/20 backdrop-blur-overlay flex items-end sm:items-center justify-center z-50" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-lg p-5 space-y-3 max-h-[90dvh] overflow-y-auto animate-scale-in" onClick={e => e.stopPropagation()}>
             <h3 className="text-lg font-bold text-slate-900">{t("ctr.new_contract_modal")}</h3>
            ...
            <div className="grid grid-cols-2 gap-2">
               <select value={form.contract_type} onChange={e => setForm(p => ({ ...p, contract_type: e.target.value }))} className="px-3 py-2.5 rounded-xl border border-slate-200 text-sm bg-white">{["service","retainer","litigation","advisory","employment"].map(cType => <option key={cType} value={cType}>{t(`ctr.type_${cType}`)}</option>)}</select>
               <select value={form.client_id} onChange={e => setForm(p => ({ ...p, client_id: e.target.value }))} className="px-3 py-2.5 rounded-xl border border-slate-200 text-sm bg-white"><option value="">{t("common.client")}...</option>{clients.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
               <select value={form.case_id} onChange={e => setForm(p => ({ ...p, case_id: e.target.value }))} className="px-3 py-2.5 rounded-xl border border-slate-200 text-sm bg-white"><option value="">{t("common.case")}...</option>{cases.map((c: any) => <option key={c.id} value={c.id}>{c.ref}</option>)}</select>
               <input type="number" value={form.value} onChange={e => setForm(p => ({ ...p, value: e.target.value }))} placeholder={t("ctr.value_label")} className="px-3 py-2.5 rounded-xl border border-slate-200 text-sm" />
