  ...
  return (
    <AppShell><div className="min-h-[100dvh] bg-transparent" dir={dir}>
      <header className="bg-white/60 glass border-b border-slate-200/60 sticky top-0 z-20 hidden md:block">
          ...
          <button onClick={() => setShowForm(true)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-500 text-white text-sm font-semibold"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" d="M12 4v16m8-8H4" /></svg>{t("comm.log_button")}</button></div>
      </header>
        ...
        {comms.map((c: any) => (
          <div key={c.id} className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4">
            <div className="flex items-center gap-2 mb-1">
              <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase ${TYPE_ICONS[c.comm_type] || "bg-gray-100 text-gray-500"}`}>{t(`comm.type_${c.comm_type}`)}</span>
              <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold ${c.direction === "inbound" ? "bg-blue-50 text-blue-600" : "bg-emerald-50 text-emerald-600"}`}>{t(`comm.direction_${c.direction}`)}</span>
              <span className="text-[10px] text-slate-400">{c.created_at}</span>
            </div>
            <p className="text-sm font-semibold text-slate-900">{c.subject || t("comm.no_subject")}</p>
            {c.body && <p className="text-xs text-slate-600 mt-1 line-clamp-2">{c.body}</p>}
            <div className="flex gap-3 mt-1 text-[10px] text-slate-400">
              {c.client_name && <span>{t("comm.client_label")}: {c.client_name}</span>}
              {c.case_ref && <span>{t("comm.case_label")}: {c.case_ref}</span>}
              {c.user_name && <span>{t("comm.by_label")}: {c.user_name}</span>}
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 bg-black/30 flex items-end sm:items-center justify-center z-50" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-md p-5 space-y-3 max-h-[90dvh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-2">
              <select value={form.comm_type} onChange={e => setForm(p => ({ ...p, comm_type: e.target.value }))} className="px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white">{["call","email","meeting","letter","sms"].map(ct => <option key={ct} value={ct}>{t(`comm.type_${ct}`)}</option>)}</select>
              <select value={form.direction} onChange={e => setForm(p => ({ ...p, direction: e.target.value }))} className="px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white"><option value="outbound">{t("comm.direction_outbound")}</option><option value="inbound">{t("comm.direction_inbound")}</option></select>
              <select value={form.client_id} onChange={e => setForm(p => ({ ...p, client_id: e.target.value }))} className="px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white"><option value="">{t("comm.client_placeholder")}</option>{clients.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
              <select value={form.case_id} onChange={e => setForm(p => ({ ...p, case_id: e.target.value }))} className="px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white"><option value="">{t("comm.case_placeholder")}</option>{cases.map((c: any) => <option key={c.id} value={c.id}>{c.ref}</option>)}</select>
            </div>
