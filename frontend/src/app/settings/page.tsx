    ...
    <AppShell><div className="min-h-[100dvh] bg-transparent">
      <header className="bg-white/60 glass border-b border-slate-200/60 sticky top-0 z-20 hidden md:block">
          <div className="px-6 flex items-center justify-between h-14">
            <h1 className="text-lg font-bold text-slate-900">Firm Settings</h1></div>
      ...
      <main className="p-3 md:p-6 max-w-5xl mx-auto space-y-4">
        {tab === "courts" && <>
          <div className="flex justify-end"><button onClick={() => setShowCourtForm(true)} className="px-4 py-2 rounded-xl bg-emerald-500 text-white text-sm font-semibold">Add Court</button></div>
          ...
          {showCourtForm && (
            <div className="fixed inset-0 bg-black/30 flex items-end sm:items-center justify-center z-50" onClick={() => setShowCourtForm(false)}>
              <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-md p-5 space-y-3" onClick={e => e.stopPropagation()}>
                ...
                <input value={courtForm.name} onChange={e => setCourtForm(p => ({ ...p, name: e.target.value }))} placeholder="Court name *" className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm" />
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <input value={courtForm.city} onChange={e => setCourtForm(p => ({ ...p, city: e.target.value }))} placeholder="City" className="px-3 py-2 rounded-lg border border-slate-200 text-sm" />
                </div>
                <div className="flex gap-2"><button onClick={() => setShowCourtForm(false)} className="flex-1 py-2 rounded-xl bg-slate-100 text-slate-600 text-sm font-semibold">Cancel</button><button onClick={addCourt} disabled={!courtForm.name} className="flex-1 py-2 rounded-xl bg-emerald-500 text-white text-sm font-semibold disabled:opacity-40">Add</button></div>
              </div>
            </div>
          )}
        </>}
        ...
        {tab === "areas" && <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
          ...
          {areas.length === 0 && <p className="text-center text-xs text-slate-400 py-8">No practice areas defined</p>}
        </div>}
        ...
        ))}{templates.length === 0 && <p className="text-center text-xs text-slate-400 py-8">No document templates</p>}</div>}
      </main>
    </div></AppShell>
  );
}
