...
export default function SettingsPage() {
  ...
  const tabs = [{ key: "courts", label: t("set.tab_courts") }, { key: "areas", label: t("set.tab_areas") }, { key: "policies", label: t("set.tab_policies") }, { key: "templates", label: t("set.tab_templates") }];
      ...
      <main className="p-3 md:p-6 max-w-5xl mx-auto space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="divide-y divide-slate-50">{courts.map((c: any) => (
              <div key={c.id} className="px-4 py-3 flex items-center justify-between">
                 <div><p className="text-sm font-semibold text-slate-800">{c.name}</p>{c.name_ar && <p className="text-xs text-slate-500">{c.name_ar}</p>}<p className="text-[10px] text-slate-400 capitalize">{t(`set.court_type_${c.court_type}`)} · {c.city}</p></div>
              </div>
            ))}</div>
          </div>
          {showCourtForm && (
            <div className="fixed inset-0 bg-black/30 flex items-end sm:items-center justify-center z-50" onClick={() => setShowCourtForm(false)}>
              <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-md p-5 space-y-3" onClick={e => e.stopPropagation()}>
                 <h3 className="text-lg font-bold text-slate-900">{t("set.add_court_modal")}</h3>
                 ...
                 <input value={courtForm.name_ar} onChange={e => setCourtForm(p => ({ ...p, name_ar: e.target.value }))} placeholder={t("set.court_name_ar_placeholder")} dir="rtl" className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm" />
                 <div className="grid grid-cols-2 gap-2">
                   <select value={courtForm.court_type} onChange={e => setCourtForm(p => ({ ...p, court_type: e.target.value }))} className="px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white">{["general","criminal","commercial","labor","administrative","appeal","supreme"].map(courtType => <option key={courtType} value={courtType}>{t(`set.court_type_${courtType}`)}</option>)}</select>
        ...
        {tab === "areas" && <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
           <div className="divide-y divide-slate-50">{areas.map((a: any) => (
             <div key={a.id} className="px-4 py-3"><p className="text-sm font-semibold text-slate-800">{a.name}</p>{a.name_ar && <p className="text-xs text-slate-500">{a.name_ar}</p>}{a.head_name && <p className="text-[10px] text-slate-400">{t("set.area_head_label")}: {a.head_name}</p>}</div>
           ))}</div>
        ...
        {tab === "templates" && <div className="space-y-2">{templates.map((t: any) => (
          <div key={t.id} className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4">
             <div className="flex items-center gap-2 mb-1"><span className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase ${t.is_active ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{t.is_active ? t("set.status_active") : t("set.status_inactive")}</span><span className="text-[10px] text-slate-400 capitalize">{t.template_type} · {t.practice_area}</span></div>
