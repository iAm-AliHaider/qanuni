    ...
    <AppShell><div className="min-h-[100dvh] bg-transparent">
      ...
      <main className="p-3 md:p-6 max-w-6xl mx-auto space-y-4 page-transition">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2 stagger">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-3 border-l-4 border-l-amber-400 animate-slide-up card-hover"><p className="text-xl font-bold text-amber-600">{stats.pending || 0}</p><p className="text-[10px] text-slate-400">Pending</p></div>
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-3 border-l-4 border-l-emerald-400 animate-slide-up card-hover"><p className="text-xl font-bold text-emerald-600">{stats.filed || 0}</p><p className="text-[10px] text-slate-400">Filed</p></div>
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-3 border-l-4 border-l-violet-400 animate-slide-up card-hover"><p className="text-xl font-bold text-violet-600">{stats.responses_due || 0}</p><p className="text-[10px] text-slate-400">Responses Due</p></div>
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-3 border-l-4 border-l-red-400 animate-slide-up card-hover"><p className="text-xl font-bold text-red-600">{stats.overdue || 0}</p><p className="text-[10px] text-slate-400">Overdue</p></div>
        </div>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      {f.response_required === 1 && <span className="px-1.5 py-0.5 rounded text-[8px] font-bold bg-orange-100 text-orange-700">Response Required</span>}
                      {isOverdue && <span className="px-1.5 py-0.5 rounded text-[8px] font-bold bg-red-100 text-red-700 animate-pulse">OVERDUE</span>}
                    </div>
                  ...
                  <div className="text-right ml-3">
                    ...
                    {f.status !== "filed" && <button onClick={() => markFiled(f.id)} className="mt-2 px-2.5 py-1 rounded-lg bg-emerald-50 border border-emerald-200 text-[10px] text-emerald-700 font-semibold">Mark Filed</button>}
                  </div>
                </div>
              </div>
            );
          })}
          {filings.length === 0 && <div className="text-center text-xs text-slate-400 py-12">No court filings</div>}
        </div>
      </main>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/20 backdrop-blur-overlay flex items-end sm:items-center justify-center z-50" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-lg p-5 space-y-3 max-h-[90dvh] overflow-y-auto animate-scale-in" onClick={e => e.stopPropagation()}>
            ...
            <input value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} placeholder="Filing title *" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm" />
            <div className="grid grid-cols-2 gap-2">
              ...
              <input value={form.najiz_ref} onChange={e => setForm(p => ({ ...p, najiz_ref: e.target.value }))} placeholder="Najiz ref" className="px-3 py-2.5 rounded-xl border border-slate-200 text-sm" />
              ...
              <input type="date" value={form.response_deadline} onChange={e => setForm(p => ({ ...p, response_deadline: e.target.value }))} className="px-3 py-2.5 rounded-xl border border-slate-200 text-sm" placeholder="Response deadline" />
            </div>
            <label className="flex items-center gap-2 text-sm text-slate-600"><input type="checkbox" checked={form.response_required} onChange={e => setForm(p => ({ ...p, response_required: e.target.checked }))} className="rounded" />Response required from opposing party</label>
            <textarea value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} placeholder="Notes" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm resize-none" rows={2} />
            <div className="flex gap-2"><button onClick={() => setShowForm(false)} className="flex-1 py-2.5 rounded-xl bg-slate-100 text-slate-600 text-sm font-semibold">Cancel</button><button onClick={create} disabled={!form.title || !form.case_id} className="flex-1 py-2.5 rounded-xl bg-emerald-500 text-white text-sm font-semibold disabled:opacity-40">Create</button></div>
          </div>
        </div>
      )}
    </div></AppShell>
