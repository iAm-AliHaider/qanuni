    ...
    <AppShell><div className="min-h-[100dvh] bg-transparent" dir={dir}>
      ...
      <main className="p-3 md:p-6 max-w-6xl mx-auto space-y-3 md:space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
           <div className="px-4 py-3 border-b border-slate-100"><h3 className="text-sm font-bold text-slate-800">{t("inv.invoices_header")}</h3></div>
          <div className="table-scroll">
          {invoices.length === 0 ? <div className="p-8 text-center text-xs text-slate-400">{t("inv.no_invoices")}</div> :
            <table className="w-full text-xs">
              <thead><tr className="bg-slate-50 text-slate-500"><th className="px-4 py-2 text-left font-medium">{t("inv.table_ref")}</th><th className="px-4 py-2 text-left font-medium">{t("inv.table_client")}</th><th className="px-4 py-2 text-left font-medium">{t("inv.table_case")}</th><th className="px-4 py-2 text-left font-medium">{t("inv.table_date")}</th><th className="px-4 py-2 text-left font-medium">{t("inv.table_status")}</th><th className="px-4 py-2 text-right font-medium">{t("inv.table_total")}</th><th className="px-4 py-2 text-center font-medium">{t("inv.table_actions")}</th></tr></thead>
              <tbody>
                    ...
                    <td className="px-4 py-2.5 text-center">
                      <div className="flex justify-center gap-1">
                         {inv.payment_status !== "paid" && (
                           <button onClick={() => { setShowPayment(inv); setPayForm(p => ({ ...p, amount: String(inv.total) })); }} className="px-2 py-1 rounded-lg bg-emerald-50 border border-emerald-200 text-[10px] text-emerald-700 font-semibold">{t("inv.pay")}</button>
                         )}
                         {inv.status === "draft" && (
                           <button onClick={() => updateStatus(inv.id, "sent")} className="px-2 py-1 rounded-lg bg-blue-50 border border-blue-200 text-[10px] text-blue-700 font-semibold">{t("inv.send")}</button>
                         )}
                      </div>
                    </td>
                  </tr>
        ...
        {showCreate && (
          <div className="fixed inset-0 bg-black/30 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4 overflow-y-auto" onClick={() => setShowCreate(false)}>
            <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-lg p-5 sm:p-6 space-y-4 max-h-[90dvh] overflow-y-auto sm:my-8" onClick={e => e.stopPropagation()}>
               <h3 className="text-lg font-bold text-slate-900">{t("inv.create_modal")}</h3>
               <div className="grid grid-cols-2 gap-3">
                 <div>
                   <label className="text-[10px] font-medium text-slate-500 mb-1 block">{t("inv.client_label")} *</label>
                 ...
                 <div>
                   <label className="text-[10px] font-medium text-slate-500 mb-1 block">{t("inv.case_label")}</label>
                 ...
                 <div>
                   <label className="text-[10px] font-medium text-slate-500 mb-1 block">{t("inv.due_date_label")}</label>
               ...
               <div>
                 <div className="flex justify-between items-center mb-2">
                   <label className="text-[10px] font-bold text-slate-500 uppercase">{t("inv.line_items_header")}</label>
                   <button onClick={() => setForm(p => ({ ...p, items: [...p.items, { description: "", quantity: 1, unit_price: 0, amount: 0, item_type: "service" }] }))} className="text-[10px] text-emerald-600 font-semibold">{t("inv.add_item_button")}</button>
                 </div>
                 {form.items.map((item: any, idx: number) => (
                   <div key={idx} className="grid grid-cols-12 gap-1 md:gap-2 mb-2 items-end">
                     <input value={item.description} onChange={e => updateItem(idx, "description", e.target.value)} placeholder={t("inv.item_description")} className="col-span-5 px-2 py-1.5 rounded border border-slate-200 text-xs" />
                     ...
                     <input type="number" value={item.unit_price} onChange={e => updateItem(idx, "unit_price", e.target.value)} placeholder={t("inv.amount_label")} className="col-span-2 px-2 py-1.5 rounded border border-slate-200 text-xs" />
               ...
               <div className="bg-slate-50 rounded-xl p-4 space-y-1">
                 <div className="flex justify-between text-xs"><span className="text-slate-500">{t("inv.subtotal_label")}</span><span className="font-medium">{subtotal.toLocaleString()} SAR</span></div>
                 <div className="flex justify-between text-xs"><span className="text-slate-500">{t("inv.vat_label")}</span><span className="font-medium">{vat.toLocaleString()} SAR</span></div>
                 <div className="flex justify-between text-sm font-bold border-t border-slate-200 pt-1 mt-1"><span>{t("inv.total_label")}</span><span className="text-emerald-600">{total.toLocaleString()} SAR</span></div>
               </div>

               <div className="flex gap-2">
                 <button onClick={() => setShowCreate(false)} className="flex-1 py-2 rounded-xl bg-slate-100 text-slate-600 text-sm font-semibold">{t("common.cancel")}</button>
                 <button onClick={createInvoice} disabled={!form.client_id || subtotal === 0} className="flex-1 py-2 rounded-xl bg-emerald-500 text-white text-sm font-semibold disabled:opacity-40">{t("inv.button_create")}</button>
               </div>
            </div>
          </div>
        )}
        ...
        {showPayment && (
          <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={() => setShowPayment(null)}>
             <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4" onClick={e => e.stopPropagation()}>
               <h3 className="text-lg font-bold text-slate-900">{t("inv.record_payment_modal")}</h3>
               <p className="text-xs text-slate-500">{t("inv.ref")}: <strong>{showPayment.ref}</strong> — {t("inv.total_label")}: <strong className="text-emerald-600">{Number(showPayment.total).toLocaleString()} SAR</strong></p>
               <div className="space-y-3">
                 <div>
                   <label className="text-[10px] font-medium text-slate-500 mb-1 block">{t("inv.amount_label")} *</label>
                 ...
                 <div>
                   <label className="text-[10px] font-medium text-slate-500 mb-1 block">{t("inv.method_label")}</label>
                   <select value={payForm.payment_method} onChange={e => setPayForm(p => ({ ...p, payment_method: e.target.value }))} className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white">
                     <option value="bank_transfer">{t("inv.bank_transfer")}</option>
                     <option value="check">{t("inv.check")}</option>
                     <option value="cash">{t("inv.cash")}</option>
                     <option value="credit_card">{t("inv.credit_card")}</option>
                   </select>
                 </div>
                 <div>
                   <label className="text-[10px] font-medium text-slate-500 mb-1 block">{t("inv.reference_label")}</label>
                   <input value={payForm.reference_number} onChange={e => setPayForm(p => ({ ...p, reference_number: e.target.value }))} placeholder={t("inv.reference_label")} className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm" />
                 </div>
                 <div>
                   <label className="text-[10px] font-medium text-slate-500 mb-1 block">{t("inv.date_label")}</label>
               ...
               <div className="flex gap-2">
                 <button onClick={() => setShowPayment(null)} className="flex-1 py-2 rounded-xl bg-slate-100 text-slate-600 text-sm font-semibold">{t("common.cancel")}</button>
                 <button onClick={recordPayment} disabled={!payForm.amount} className="flex-1 py-2 rounded-xl bg-emerald-500 text-white text-sm font-semibold disabled:opacity-40">{t("inv.button_record_payment")}</button>
               </div>
            </div>
          </div>
        )}
