  ...
  return (
    <AppShell><div className="min-h-[100dvh] bg-transparent" dir={dir}>
      ...
      <main className="p-3 md:p-6 max-w-5xl mx-auto space-y-4 page-transition">
         <div className="flex items-center justify-between md:hidden"><h1 className="text-lg font-bold text-slate-900">{t("ret.mobile_header")}</h1><button onClick={() => setShowForm(true)} className="px-3 py-1.5 rounded-xl bg-emerald-500 text-white text-xs font-semibold">+ New</button></div>

         <div className="grid grid-cols-3 gap-2 stagger">
           <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-3 border-l-4 border-l-blue-400 animate-slide-up card-hover"><p className="text-xl font-bold text-blue-600">{stats.total || 0}</p><p className="text-[10px] text-slate-400">{t("ret.total_label")}</p></div>
           <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-3 border-l-4 border-l-emerald-400 animate-slide-up card-hover"><p className="text-xl font-bold text-emerald-600">{stats.active || 0}</p><p className="text-[10px] text-slate-400">{t("ret.active_label")}</p></div>
               ...
               <div className="flex items-start justify-between">
                 <div>
                   <div className="flex items-center gap-2 mb-1">
                     <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase ${TYPE_COLORS[r.agreement_type] || "bg-gray-100 text-gray-500"}`}>{t(`ret.type_${r.agreement_type}`)}</span>
                     <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase ${STATUS_COLORS[r.status] || "bg-gray-100 text-gray-500"}`}>{t(`common.${r.status}`)}</span>
                   </div>
                    <p className="text-sm font-semibold text-slate-900">{r.client_name || t("common.unknown")}</p>
                   <div className="flex gap-3 mt-1 text-[10px] text-slate-400">
                     {r.case_ref && <span>{t("ret.case_label")}: {r.case_ref}</span>}
                     <span>{r.start_date} {t("ret.date_range_separator")} {r.end_date || t("common.ongoing")}</span>
                     <span>{t("ret.billing_day")}: {r.billing_day}</span>
                   </div>
                 </div>
                 <div className="text-right">
                   <p className="text-sm font-bold text-slate-800">{Number(r.amount).toLocaleString()} SAR</p>
                   <p className="text-[10px] text-slate-400">{t("ret.per_monthly")}{r.agreement_type === "monthly" ? "" : r.agreement_type === "quarterly" ? t("ret.per_quarterly") : r.agreement_type === "annual" ? t("ret.per_annual") : t("ret.per_project")}</p>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/20 backdrop-blur-overlay flex items-end sm:items-center justify-center z-50" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-sm p-5 space-y-3 animate-scale-in" onClick={e => e.stopPropagation()}>
             <h3 className="text-lg font-bold text-slate-900">{t("ret.new_retainer_modal")}</h3>
             <select value={form.client_id} onChange={e => setForm(p => ({ ...p, client_id: e.target.value }))} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm bg-white"><option value="">{t("ret.select_client")} *</option>{clients.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
             <div className="grid grid-cols-2 gap-2">
               <select value={form.agreement_type} onChange={e => setForm(p => ({ ...p, agreement_type: e.target.value }))} className="px-3 py-2.5 rounded-xl border border-slate-200 text-sm bg-white">{["monthly","quarterly","annual","project"].map(agreeType => <option key={agreeType} value={agreeType}>{t(`ret.type_${agreeType}`)}</option>)}</select>
               <input type="number" value={form.amount} onChange={e => setForm(p => ({ ...p, amount: e.target.value }))} placeholder={t("ret.amount") + " *"} className="px-3 py-2.5 rounded-xl border border-slate-200 text-sm" />
