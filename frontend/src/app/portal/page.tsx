      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-emerald-50/30 to-teal-50/20 flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-white rounded-3xl shadow-xl p-8 space-y-6">
          <div className="text-center">
            ...
            <p className="text-sm text-slate-500">بوابة العملاء — View your cases & invoices</p>
          </div>
          <div className="space-y-3">
            ...
            <input type="password" value={pin} onChange={e => setPin(e.target.value)} onKeyDown={e => e.key === "Enter" && login()} placeholder="Enter PIN" maxLength={4} className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm text-center tracking-[0.5em] focus:ring-2 focus:ring-emerald-200 focus:border-emerald-400 outline-none"/>
            {error && <p className="text-red-500 text-xs text-center">{error}</p>}
            <button onClick={login} disabled={!selectedId || pin.length < 4} className="w-full py-3 rounded-xl bg-emerald-500 text-white font-semibold hover:bg-emerald-600 transition-colors disabled:opacity-50">Sign In</button>
          </div>
          <p className="text-center text-[10px] text-slate-300">Al-Rashid & Partners Law Firm — Powered by Qanuni</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="bg-white border-b border-slate-200 px-4 md:px-8 py-4 flex items-center justify-between">
        ...
        <button onClick={logout} className="px-3 py-1.5 rounded-lg bg-slate-100 text-xs text-slate-600 hover:bg-slate-200">Sign Out</button>
      </header>

      <div className="max-w-5xl mx-auto p-4 md:p-8 space-y-6">
        {/* Tabs */}
        <div className="flex gap-1 bg-white rounded-xl p-1 border border-slate-200/80 shadow-sm overflow-x-auto">
          {(["overview", "cases", "invoices", "hearings", "documents"] as const).map(t => (
            <button key={t} onClick={() => loadTab(t)} className={`px-4 py-2 rounded-lg text-sm font-semibold whitespace-nowrap transition-colors ${tab === t ? "bg-emerald-500 text-white" : "text-slate-500 hover:bg-slate-50"}`}>
        ...
        {tab === "overview" && overview && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: "Active Cases", value: overview.cases?.active || 0, color: "emerald" },
                { label: "Total Billed", value: `SAR ${Number(overview.billing?.total || 0).toLocaleString()}`, color: "blue" },
                { label: "Outstanding", value: `SAR ${Number(overview.billing?.outstanding || 0).toLocaleString()}`, color: "amber" },
                { label: "Paid", value: `SAR ${Number(overview.billing?.paid || 0).toLocaleString()}`, color: "emerald" },
              ].map((kpi, i) => (
            ...
            {(overview.upcomingHearings || []).length > 0 && (
              <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-5">
                <h3 className="text-xs font-bold text-slate-500 uppercase mb-3">Upcoming Hearings — الجلسات القادمة</h3>
        ...
        {tab === "cases" && tabData && (
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            {!(tabData.cases||[]).length && <div className="text-center py-10 text-slate-300">No cases</div>}
          </div>
        )}
        ...
        {tab === "invoices" && tabData && (
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            {!(tabData.invoices||[]).length && <div className="text-center py-10 text-slate-300">No invoices</div>}
          </div>
        )}
        ...
        {tab === "hearings" && tabData && (
          <div className="space-y-3">
            {(tabData.hearings || []).length === 0 && <div className="text-center py-10 text-slate-300">No upcoming hearings</div>}
        ...
        {tab === "documents" && tabData && (
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            {!(tabData.documents||[]).length && <div className="text-center py-10 text-slate-300">No documents</div>}
          </div>
        )}
      </div>

      <footer className="text-center py-6 text-[10px] text-slate-300">مكتب الراشد والشركاء للمحاماة — Powered by Qanuni</footer>
    </div>
  );
}
