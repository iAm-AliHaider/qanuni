    ...
    <AppShell><div className="min-h-[100dvh] bg-transparent">
      ...
      <main className="p-3 md:p-6 max-w-5xl mx-auto space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-5">
          <h3 className="text-sm font-bold text-slate-800 mb-3">{t("comp.conflict_check")}</h3>
          <div className="flex gap-2">
            <input value={checkName} onChange={e => setCheckName(e.target.value)} onKeyDown={e => e.key === "Enter" && runCheck()} placeholder={t("comp.enter_party")} className="flex-1 px-3 py-2 rounded-lg border border-slate-200 text-sm" />
            <button onClick={runCheck} disabled={!checkName.trim() || checking} className="px-5 py-2 rounded-xl bg-slate-900 text-white text-sm font-semibold disabled:opacity-40">{checking ? t("comp.checking") : t("comp.run_check")}</button>
          </div>
          {checkResult && (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-3 border-l-4 border-l-amber-400"><p className="text-xl font-bold text-amber-600">{pendingKyc.length}</p><p className="text-[10px] text-slate-400">{t("comp.pending_kyc")}</p></div>
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-3 border-l-4 border-l-red-400"><p className="text-xl font-bold text-red-600">{highRisk.length}</p><p className="text-[10px] text-slate-400">{t("comp.high_risk")}</p></div>
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-3 border-l-4 border-l-blue-400"><p className="text-xl font-bold text-blue-600">{conflicts.length}</p><p className="text-[10px] text-slate-400">{t("comp.recent_checks").split(" ").slice(0,2).join(" ")}</p></div>
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-3 border-l-4 border-l-violet-400"><p className="text-xl font-bold text-violet-600">{expiringPoa.length}</p><p className="text-[10px] text-slate-400">{t("comp.poas_expiring")}</p></div>
        </div>

        {pendingKyc.length > 0 && <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
          <div className="px-4 py-2.5 border-b border-slate-100 bg-amber-50"><h3 className="text-xs font-bold text-amber-800 uppercase">{t("comp.kyc_verify")}</h3></div>
