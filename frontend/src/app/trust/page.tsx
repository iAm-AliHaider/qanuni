    ...
    <AppShell><div className="min-h-[100dvh] bg-transparent">
      ...
      <main className="p-3 md:p-6 max-w-6xl mx-auto space-y-4 page-transition">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
          <div className="px-4 py-2.5 border-b border-slate-100"><h3 className="text-sm font-bold text-slate-800">{t("trust.client_trust")}</h3></div>
            ...
            {accounts.map((a: any) => (
              <div key={a.id} className="px-4 py-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setShowDeposit({ account_id: a.id, type: "deposit" })} className="px-2 py-1 rounded-lg bg-emerald-50 border border-emerald-200 text-[9px] text-emerald-700 font-semibold">{t("common.deposit")}</button>
                  <button onClick={() => setShowDeposit({ account_id: a.id, type: "withdrawal" })} className="px-2 py-1 rounded-lg bg-red-50 border border-red-200 text-[9px] text-red-700 font-semibold">{t("common.withdraw")}</button>
                </div>
              </div>
            ))}
            {accounts.length === 0 && <p className="px-4 py-8 text-center text-xs text-slate-400">{t("trust.no_accounts")}</p>}
          </div>
        </div>

        {/* Recent transactions */}
        {transactions.length > 0 && <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
          <div className="px-4 py-2.5 border-b border-slate-100"><h3 className="text-sm font-bold text-slate-800">{t("trust.recent_tx")}</h3></div>
          <div className="divide-y divide-slate-50">
