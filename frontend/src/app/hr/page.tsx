...
// ROLE_LABELS replaced with t("role.xxx")
      ...
      <main className="p-3 md:p-6 max-w-6xl mx-auto space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          ...
          {byRole.slice(0, 3).map((r: any) => (
             <div key={r.role} className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-3"><p className="text-xl font-bold text-slate-700">{r.count}</p><p className="text-[10px] text-slate-400 capitalize">{t(`role.${r.role}`)}</p></div>
          ))}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4">
            ...
            <div className="space-y-1">{byRole.map((r: any) => (
               <div key={r.role} className="flex items-center justify-between p-2 rounded-lg hover:bg-slate-50"><span className={`px-2 py-0.5 rounded text-[9px] font-bold ${ROLE_COLORS[r.role] || "bg-gray-100 text-gray-500"}`}>{t(`role.${r.role}`)}</span><span className="text-xs font-bold text-slate-500">{r.count}</span></div>
            ))}</div>
          </div>
        </div>
            ...
            <div key={l.id} className="px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-500">{l.name?.split(" ").map((n: string) => n[0]).join("").slice(0, 2)}</div>
                 <div><p className="text-sm font-semibold text-slate-800">{l.name}</p><div className="flex items-center gap-2 mt-0.5"><span className={`px-1.5 py-0.5 rounded text-[8px] font-bold ${ROLE_COLORS[l.role] || "bg-gray-100 text-gray-500"}`}>{t(`role.${l.role}`)}</span><span className="text-[10px] text-slate-400">{l.department}</span>{l.bar_number && <span className="text-[10px] text-slate-400 font-mono">{l.bar_number}</span>}</div></div>
              </div>
